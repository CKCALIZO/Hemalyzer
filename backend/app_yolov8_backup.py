...
def process_blood_smear(image_bytes, conf_threshold=0.01, iou_threshold=0.2):
    ...
    - conf_threshold: 0.01 (detect everything, filter later if needed)
    - iou_threshold: 0.2 (20% overlap - less aggressive NMS to keep more detections)
    
    Args:
        image_bytes: Raw image bytes
        conf_threshold: Detection confidence threshold (default 0.01 for maximum detection)
        iou_threshold: IoU threshold for NMS (default 0.2 = 20% overlap threshold)
        
    Returns:
        dict: Analysis results with detections and classifications
    """
    # OPTIMIZED: Use very low confidence to detect maximum cells
    # IoU at 0.2 (20%) means boxes need 80%+ unique area to both survive NMS
    # This allows more overlapping detections through
    ...
    try:
        ...
        # OPTIMIZED FOR MAXIMUM BLOOD CELL DETECTION:
        # - conf=0.01: Detect everything (confidence filtering disabled)
        # - iou=0.2: 20% overlap threshold (minimal NMS suppression)
        # - max_det=3000: Handle very dense blood smear images
        # - agnostic_nms=True: Don't suppress across different cell types
        results = yolo_model.predict(
            source=image_np,
            conf=conf_threshold,      # 0.01 default - detects all possible cells
            iou=iou_threshold,        # 0.2 default - 20% overlap threshold
            imgsz=640,
            max_det=3000,             # Increased for dense cell images
            agnostic_nms=True,        # Don't suppress across classes (WBC/RBC/Platelets)
        ...
        for box in boxes:
            ...
            if 'PLATELET' in cls_name_normalized:
                ...
                # WBC VALIDATION: Size-based only (confidence disregarded for max detection)
                # WBCs are typically 2-3x larger than RBCs (area > 600 is reasonable)
                # Lowered area threshold slightly to catch more WBCs
                if area > 600:
                    detections['counts']['WBC'] += 1
                    wbc_boxes.append((box, detection))
                    cell_type = 'WBC (validated)'
                else:
                    # Likely a misclassified RBC - reclassify as RBC based on size
                    ...
                    detection['class'] = 'RBC'
                    print(f"   ⚠️ Rejected WBC (too small): area={area:.0f}, conf={confidence:.3f}")
            elif 'RBC' in cls_name_normalized or 'REDBLOODCELL' in cls_name_normalized:
                detections['counts']['RBC'] += 1
                cell_type = 'RBC'
            else:
...
def analyze_blood_smear():
    ...
    try:
        ...
        # OPTIMIZED: Very low confidence (0.01) to detect maximum cells
        # IoU threshold at 0.2 (20%) for minimal NMS suppression
        conf_threshold = float(request.form.get('conf_threshold', 0.01))
        iou_threshold = float(request.form.get('iou_threshold', 0.2))  # 20% overlap threshold
...
def models_info():
    ...
    return jsonify({
        ...
        'default_params': {
            'conf_threshold': 0.01,  # Very low for maximum detection
    ...
    try:
        ...
        for conf in test_thresholds:
            results = yolo_model.predict(
                source=image_np,
                conf=conf,
                iou=0.2,  # 20% overlap threshold for maximum detection
                imgsz=640,
                max_det=3000,  # Increased for dense images
                agnostic_nms=True,
                device='cuda' if torch.cuda.is_available() else 'cpu',
                verbose=False
            )
